...
public class CourseManager implements CourseService {
    ...
    @Autowired
    public CourseManager(@Qualifier("courseRepositoryHibernateImplement") CourseRepository courseRepository) {
    ...
    public List<CourseViewDto> getCourses() {
        return ((List<Course>)this.courseRepository.findAll()).stream().map(CourseViewDto::of).collect(Collectors.toList());
    }

    @Override
    public CourseViewDto getCourseById(Long id) {
        Course course = (Course) this.courseRepository.findById(id);
        return CourseViewDto.of(course);
    }

    @Override
    public CourseViewDto createCourse(CourseCreateDto courseCreateDto) {
        final Course course = (Course) this.courseRepository.save(new Course(courseCreateDto.getName(), courseCreateDto.getCourseCode(), courseCreateDto.getCredit(), courseCreateDto.getStudents(), courseCreateDto.getInstructor()));
        return CourseViewDto.of(course);
    }
    ...
    public CourseViewDto updateCourse(Long id, CourseUpdateDto courseUpdateDto) {
        Course oldCourse = (Course) this.courseRepository.findById(id);
        ...
        Course newCourse = (Course) this.courseRepository.save(oldCourse);
